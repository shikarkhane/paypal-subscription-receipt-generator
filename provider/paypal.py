...
class Plan:
    def __init__(self, json: dict):
        ...
        self.usage_type = json["usage_type"]
        self.plan_detail_link = json["links"][0]["href"]
...
class Transaction:
    ...
    def repr_json(self):
        return dict(
            payer_email_address=self.payer_email_address,
            paypal_reference_id=self.paypal_reference_id,
...
class Gateway:
    def __init__(self):
        ...
        response = requests.post(f"{self.base_url}/oauth2/token", headers=access_token_headers, data=data, auth=auth)
    ...
    def get_plans(self):
        ...
        res = requests.get(
            f"{self.base_url}/billing/plans?page_size=20&page=1&total_required=true",
            headers=transaction_headers,
        )

        plans = [Plan(i) for i in res.json()["plans"]]

        for plan in plans:
            if plan.status == "ACTIVE":
                res = requests.get(
                    plan.plan_detail_link,
                    headers=transaction_headers,
                )
