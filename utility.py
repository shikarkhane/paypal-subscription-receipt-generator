...
def get_html_template(use_with_vat="no"):
    template_to_use = "template.html"
    with_vat_template = "with_vat_template.html"

    if use_with_vat == "yes":
        template_to_use = with_vat_template

    templates_dir = os.path.join(root, "templates")
    env = Environment(loader=FileSystemLoader(templates_dir))
    return env.get_template(template_to_use)
...
class PdfWriter:
    ...
    def write_to_pdf(self, payee, payer, transaction, with_vat="no"):
        ...
        with open(filename, "w") as fh:
            fh.write(
                get_html_template(with_vat).render(
                    ...
                    vat_price=f"{transaction.currency_symbol} {round(float(transaction.payment_amount)/1.25 * .25)}",
    ...
    def html_to_pdf(self, html_file_name):
        filename = os.path.join(root, "receipts/HTML", f"{html_file_name}.html")

        pdf_filename = os.path.join(root, "receipts/PDF", f"{html_file_name}.pdf")
